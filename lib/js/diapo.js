	...
	$.fn.diapo = function (opts, callback) {
			...
			if (navHover == true) { //if the option is true I show the next and prev button only on mouse over
				if (isMobile()) {
					elem.live('vmouseover', function () {
						$('#pix_prev, #pix_next').animate({
							opacity: 1
						}, 200);
					});
					elem.live('vmouseout', function () {
						$('#pix_prev, #pix_next').delay(500).animate({
							opacity: 0
						}, 200);
					});
				} else {
					elem.hover(function () {
						$('#pix_prev, #pix_next').stop(true, false).animate({
							opacity: 1
						}, 200);
					}, function () {
						$('#pix_prev, #pix_next').stop(true, false).animate({
		...
		if (pagination == true) {
			...
			for (li = 0; li < amountSlide; li++) {
				...
				if (opts.thumbs == true) {
					...
					$('li#pag_nav_' + li).append($(newImg).attr('class', 'pix_thumb').css('position', 'absolute').animate({
						opacity: 0
					}, 0));
					$('li#pag_nav_' + li + ' > img').after('<div class="thumb_arrow" />');
					$('li#pag_nav_' + li + ' > .thumb_arrow').animate({
						opacity: 0
					}, 0);

					if (!isMobile()) {
						$('#pix_pag_ul,#pix_pag_ul li,#pix_pag_ul li span,#pix_pag_ul li img,#pix_pag_ul li div').hover(function(){
							// elem.diapoStop();
							elem.addClass('stopped');
						},function(){
							// elem.diapoPlay();
							if (autoAdv != false) elem.removeClass('stopped');
						})
						
						$('#pix_pag li').hover(function () {
							elem.addClass('stopped');
							$('.pix_thumb, .thumb_arrow', this).addClass('visible').stop(true, false).animate({
								'margin-top': -15,
								opacity: 1
							}, 300, 'easeOutQuad');
						}, function () {
							if (autoAdv != false) elem.removeClass('stopped');
							$('.pix_thumb, .thumb_arrow', this).removeClass('visible').stop(true, false).animate({
								'margin-top': -12,
								opacity: 0
							}, 150);
						});
		...
		function nextSlide(nav) { //funzione per il fading delle immagini
			...
			var rows = opts.rows,
				...
				marginLeft = 0,
				marginTop = 0;
			var randomFx = new Array('simpleFade', 'curtainTopLeft', 'curtainTopRight', 'curtainBottomLeft', 'curtainBottomRight', 'curtainSliceLeft', 'curtainSliceRight', 'blindCurtainTopLeft', 'blindCurtainTopRight', 'blindCurtainBottomLeft', 'blindCurtainBottomRight', 'blindCurtainSliceBottom', 'blindCurtainSliceTop', 'stampede', 'mosaic', 'mosaicReverse', 'mosaicRandom', 'mosaicSpiral', 'mosaicSpiralReverse', 'topLeftBottomRight', 'bottomRightTopLeft', 'bottomLeftTopRight', 'bottomLeftTopRight', 'scrollLeft', 'scrollRight', 'scrollTop', 'scrollBottom', 'scrollHorz');
			...
			$.each(arr, function (index, value) {
				...
				function diapoeased() {
					...
					if ($('.diapoeased').length == blocks) {
						...
						if (lMoveIn != 0) {
							$('.diapocurrent .fromLeft, .diapocurrent .fromRight, .diapocurrent .fromTop, .diapocurrent .fromBottom, .diapocurrent .fadeIn').each(function (i,e) {
								var easeMove = easing;
								if ($(this).attr('data-easing') != '') easeMove = $(this).attr('data-easing');
								var t = $(this);
								String.prototype._toNum = function(){
								...
								var top = pos.top;
								var origonCss = {
									l:t.css('left')._toNum() == left ? left : t.css('left')._toNum(),
									t:t.css('top')._toNum() == top ? top : t.css('top')._toNum(),
									r:t.css('right')._toNum(),
									b:t.css('bottom')._toNum(),
									w:t.css('width')._toNum(),
									h:t.css('height')._toNum()
								} 
								var wMoveIn = t.width();
								...
								if (tClass.indexOf("fromLeft") != -1) {
									...
									t.css('visibility', 'visible').delay((time / lMoveIn) * (0.1 * (ind - 1))).animate({
										'left': origonCss.l
									}, (time / lMoveIn) * 0.2, easeMove);
								...
								if (($.browser.msie && $.browser.version < 9) || opts.loader == 'bar') {
									...
									if (rad <= 1 && !elem.hasClass('stopped')) {
										...
										if (!elem.hasClass('stopped'))
											imgFake();
										clearInterval(u);
										$('#pix_canvas, #pix_canvas_wrap').animate({
											opacity: 0
										}, 200, function () {
											setTimeout(canvasLoader, opts.transPeriod + difference);
											nextSlide();
